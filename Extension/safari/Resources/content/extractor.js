...
(() => {
  ...
  /** Extract table data as array-of-arrays (headers + rows). */
  ...
  function extractList(selector) {
    const list = document.querySelector(selector || "ul, ol");
    if (!list) return { error: "No list found" };

    const items = [];
    for (const li of list.querySelectorAll("li")) {
      items.push(compactText(li.innerText || li.textContent || ""));
    }
  ...
  browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
    try {
      switch (message.method) {
        ...
        case "dom.extractTable":
          sendResponse({ result: extractTable(message.params?.selector) });
          break;

        case "dom.extractList":
          sendResponse({ result: extractList(message.params?.selector) });
          break;

        case "dom.extractLinks":
          sendResponse({ result: extractLinks(message.params?.selector) });
          break;
